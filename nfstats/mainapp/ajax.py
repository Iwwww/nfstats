...
from .functions import (
    generate_ip_flows_data,
    generate_as_flows_data,
    generate_interface_flows_sum,
    generate_interface_flows_data,
    generate_ip_traffic_data,
    get_shell_data,
    put_interface_names,
)
import json
...
def get_pie_chart_data(request):
    if request.POST:
        ...
        if request.POST["aggregate"]:
            ...
            Aggregate_interface = namedtuple(
                "Aggregate_interface", "name description snmpid"
            )
            name_aggregate = ""
            for snmpid in snmpid_aggregate:
                name_aggregate += (
                    Interface.objects.get(
                        host__host=host, sampling=True, snmpid=snmpid
                    ).name
                    + ", "
                )
            interfaces = [
                Aggregate_interface(
                    name=name_aggregate[:-2],
        ...
        for interface in interfaces:
            ...
            try:
                if snmpid_aggregate:
                    ...
                    for snmpid_part in snmpid_aggregate:
                        speed_data_aggregate["in_bps"] += Speed.objects.get(
                            date=date_db, interface__snmpid=snmpid_part
                        ).in_bps
                        speed_data_aggregate["out_bps"] += Speed.objects.get(
                            date=date_db, interface__snmpid=snmpid_part
                        ).out_bps
                    speed_data = Aggregate_speed(
                        speed_data_aggregate["in_bps"], speed_data_aggregate["out_bps"]
                    )
                else:
                    speed_data = Speed.objects.get(date=date_db, interface=interface)
            except Speed.DoesNotExist:
                logger.error(
                    f"Speed data for the interface {interface} and date: {date_db} does not exist"
                )
                data[snmpid][
                    "error"
                ] = f"Speed data for the interface {interface} and date: {date_db} does not exist"
            ...
            for as_type in ["source", "destination"]:
                ...
                try:
                    flows_data = generate_interface_flows_data(
                        direction,
                        direction,
                        date,
                        host,
...
def get_interface_chart_data(request):
    ...
    data = [
        [
            {"label": "Point 1", "type": "string"},
            {"label": "Point 2", "type": "string"},
    ...
    for intrf, octets in flows_sum:
        ...
        try:
            ...
            result = JsonResponse(
                {
                    "error": f"Speed data for the interface {interface} and date: {date_db} does not exist"
                }
            )
            result.status_code = 500
            return result
        factor = (
            speed_data.in_bps / (int(octets) * 1000000)
            if filter_direction == "output"
            else speed_data.out_bps / (int(octets) * 1000000)
        )
        speed_factor[intrf] = factor
    for as_type in ["source", "destination"]:
        try:
            flows_data = generate_interface_flows_data(
                filter_direction, report_direction, date, host, snmpid, as_type
            )
        except Exception as e:
            result = JsonResponse({"error": str(e)})
            result.status_code = 500
        ...
        for intrf, as_bgp, octets in flows_data:
            ...
            (
                data.append(["sAS" + as_bgp, intrf, speed])
                if as_type == "source"
                else data.append([intrf, "dAS" + as_bgp, speed])
            )
    return HttpResponse(json.dumps(data))
...
def get_as_chart_data(request):
    if request.POST:
        ...
        for in_intrf, out_intrf, sas, das, octets in flows_data:
            speed = (
                round(speed_factor[in_intrf] * int(octets), 2)
                if direction == "input"
                else round(speed_factor[out_intrf] * int(octets), 2)
            )
...
def get_ip_chart_data(request):
    ...
    try:
        ...
        return result
    try:
        flows_data = generate_ip_flows_data(
            direction,
            date,
            host,
            snmpid_smpl,
...
def get_ip_traffic_data(request):
    ...
    snmpid_nsmpl = request.POST["interface"]
    try:
        flows_data = generate_ip_traffic_data(
            direction,
            date,
            host,
            snmpid_smpl,
    ...
    try:
        with open(
            Path(settings.VARS["data_dir"])
            .joinpath(f"ip_traffic_data_{request.session['session_id']}.csv")
            .resolve(),
            "w",
            encoding="utf8",
        ) as f:
...
def download_ip_traffic_data(request):
    ...
    ip = request.GET["ip"]
    file_path = Path(settings.VARS["data_dir"]).joinpath(
        f"ip_traffic_data_{request.session['session_id']}.csv"
    )
    try:
        with open(file_path, "rb") as f:
            response = HttpResponse(f.read(), content_type="text/csv")
            response["Content-Disposition"] = (
                "attachment; filename=" + ip + "_" + date + ".csv"
            )
            return response
    except Exception as e:
        logger.error(f"(File R/W): {e}")
