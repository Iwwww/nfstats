...
from .utils import update_globals
import json
import logging
...
def get_snmp_interfaces(request):
    ...
    command = f"{settings.VARS['snmp_walk']} -v{settings.SYS_SETTINGS['snmp_ver']} -Oseqn -c {host_obj.snmp_com} {host} .1.3.6.1.2.1.2.2.1.2"
    try:
        ...
        return result
    command = f"{settings.VARS['snmp_walk']} -v{settings.SYS_SETTINGS['snmp_ver']} -Oseqn -c {host_obj.snmp_com} {host} .1.3.6.1.2.1.31.1.1.1.18"
    try:
...
def add_interface(request):
    ...
    try:
        ...
        obj = Interface(
            snmpid=int(snmpid), name=name, description=description, host=host_obj
        )
        obj.save()
        ...
        result = JsonResponse(
            {"error": f"Error: (DB): Interface already exist ({host} snmpid: {snmpid})"}
        )
        result.status_code = 500
        return result
...
def update_interface(request):
    ...
    try:
        ...
        logger.error(
            f"(DB): Interface does not exist ({interface_id} snmpid: {snmpid})"
        )
        result = JsonResponse(
            {
                "error": f"Error: (DB): Interface does not exist ({interface_id} snmpid: {snmpid})"
            }
        )
        result.status_code = 500
        return result
...
def update_interface_sampling(request):
    ...
    try:
        ...
        logger.error(f"(DB): Interface does not exist ({interface_id})")
        result = JsonResponse(
            {"error": f"Error: (DB): Interface does not exist ({interface_id})"}
        )
        result.status_code = 500
        return result
    else:
        setattr(obj, "sampling", sampling == "true")
        obj.save()
        result = JsonResponse({"result": "Interface updated"})
        result.status_code = 200
        return result
...
def add_host(request):
    ...
    try:
        ...
        result = JsonResponse(
            {"error": f"Error: (DB): Host already exist ({host} name: {name})"}
        )
        result.status_code = 500
        return result
...
def update_host(request):
    ...
    try:
        ...
        result = JsonResponse(
            {"error": f"Error: (DB): Host does not exist ({host} name: {name})"}
        )
        result.status_code = 500
        return result
    except Exception as e:
...
def delete_host(request):
    ...
    try:
        ...
        logger.error(f"(DB): Host does not exist ({host_id})")
        result = JsonResponse(
            {"error": f"Error: (DB): Host does not exist ({host_id})"}
        )
        result.status_code = 500
        return result
    else:
