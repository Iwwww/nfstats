...
from nfstats.settings import VARS
...
def create_nfdump_filter(
    interfaces_smpl,
    direction_key,
    snmpid_nsmpl=None,
    direction_key_nsmpl=None,
    src_as=None,
    dst_as=None,
    src_port=None,
    dst_port=None,
    ...
    if interfaces_smpl:
        ...
        logger.error(
            f"Not found 'sampling' interfaces in the DB. Check at least one interface as 'sampling' in the settings!"
        )
        raise Exception(
            f"Error: Not found 'sampling' interfaces in the DB. Check at least one interface as 'sampling' in the settings!"
        )
    if snmpid_nsmpl:
...
def generate_interface_flows_data(
    filter_direction,
    report_direction,
    date,
    host,
    ...
    if filter_direction == report_direction:
        if snmpid_aggregate:
            ...
            command = (
                f"{VARS['nfdump']} -r {flows_file} -A {as_type_key_filter}as "
            ...
            command = (
                f"{VARS['nfdump']} -r {flows_file} -A {as_type_key_filter}as,{direction_key_report}if "
...
def generate_interface_flows_sum(direction, date, host):
    ...
    command = (
        f"{VARS['nfdump']} -r {flows_file} -A {direction_key}if "
...
def generate_as_flows_data(direction, date, host):
    ...
    filter_keys = create_nfdump_filter(interfaces, direction_key)
    command = (
        f"{VARS['nfdump']} -r {flows_file} -A inif,outif,srcas,dstas "
...
def generate_ip_flows_data(
    ...
    if snmpid_smpl:
        interfaces = Interface.objects.filter(
            host__host=host, sampling=True, snmpid=int(snmpid_smpl)
        ).all()
    else:
        interfaces = Interface.objects.filter(host__host=host, sampling=True).all()
    flows_file = get_flows_file(host, date)
    ...
    filter_keys = create_nfdump_filter(
        interfaces,
        direction_key,
        snmpid_nsmpl,
        direction_key_nsmpl,
    ...
    command = (
        f"{VARS['nfdump']} -r {flows_file} -A {ip_type_key}ip,{direction_key}if "
...
def generate_ip_traffic_data(
    ...
    result = get_shell_data(
        command,
        r"\d+\-\d+\-\d+\s+(\d+:\d+:\d+).\d+,\s*\d+\-\d+\-\d+\s+(\d+:\d+:\d+).\d+,\s*(\d+),\s*(\d+.\d+.\d+.\d+),\s*(\d+),\s*(\d+),\s*(\d+.\d+.\d+.\d+),\s*(\d+),\s*(\d+)\s*,\s*(\d+),\s*(\d+),\s*(\d+)",
    )
    return result
